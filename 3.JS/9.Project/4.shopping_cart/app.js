...
app.delete('/api/delete-cart/:itemId', (req, res) => {
    const deleteItemId = Number(req.params.itemId);
    const cart = req.session.cart || [];
    console.log("카트확인:", cart);
    const deleteItem = cart.find((item) => item.id === deleteItemId);
    // console.log(deleteItem);
    if (deleteItem) {
        new_cart = cart.filter((item) => item.id !== deleteItemId);
